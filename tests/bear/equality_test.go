...
func TestEqualVariants_StaleVsFreshURL(t *testing.T) {
	...
	if !domain.EqualIgnoringNewNoteLinkForTest(stale, fresh) {
		t.Errorf("non-strict: stale vs fresh should compare equal (URL drift ignored)")
	}
	if domain.EqualIgnoringNewNoteLinkStrictForTest(stale, fresh) {
	...
	if !domain.EqualIgnoringNewNoteLinkForTest(stale, sameStale) {
		t.Errorf("non-strict: byte-identical URLs should equal")
	}
	if !domain.EqualIgnoringNewNoteLinkStrictForTest(stale, sameStale) {

// Package bear_test residue tests — exercises bear.AggregateUntrackedFromJSON
...
func TestScanUntrackedMixedTagsCountsBothScopes(t *testing.T) {
	...
	report, err := bear.AggregateUntrackedFromJSON(mustMarshalNotes(t, notes), managedSet("library"))
	if err != nil {
		t.Fatalf("AggregateUntrackedFromJSON: %v", err)
	}
	if got, want := len(report.TagFamilies), 2; got != want {
...
func TestScanUntrackedSortStability(t *testing.T) {
	...
	report, err := bear.AggregateUntrackedFromJSON(mustMarshalNotes(t, notes), managedSet())
	if err != nil {
		t.Fatalf("AggregateUntrackedFromJSON: %v", err)
	}
...
func tagsOf(families []bear.UntrackedFamily) []string {
